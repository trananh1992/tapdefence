package com.kulinich.tapdefence.graphics;
...
public class GameSurface extends SurfaceView implements SurfaceHolder.Callback {
	...
	public class GameThread extends Thread {
		...
		private float PARTICLES_PER_POINT = 5;
		...
		private void doDraw(Canvas canvas) {
			...
			// Rumble
			if(mRumbleAmount > 0){						
				canvas.translate((float) Math.floor(Math.random() * 
								(mRumbleAmount/4 + 1)) - mRumbleAmount/8, 
								(float) Math.floor(Math.random() * 
								(mRumbleAmount/4) - mRumbleAmount/8));
			}
			...
			mScratchRect.set(4, 4, 4 + fuelGhostWidth, 4 + UI_BAR_HEIGHT);
		...
		private void updateDifficulty(double elapsed) {
			...
			if (ENEMY_PERIOD < ENEMY_MIN_PERIOD) {
				...
				Log.d("Diff", "Enemy period already min");
			}
				...
				try {
					for (int j = 0; j < mWalls.size(); ++j) {
						...
						if (intersection != null) {
							...
							makeDebris(intersection, mWallPaint);
		...
		private void makeDebris(Line line, Paint paint) {
			...
			for (int i = 0; i < count; ++i) {
				...
				debris.x = line.a.x + dx * i + 
						randsgn() * (float) Math.random() * PARTICLE_SCATTER;
				debris.y = line.a.y + dy * i + 
						randsgn() * (float) Math.random() * PARTICLE_SCATTER;
		...
		private void makeDebris(Point point, Paint paint) {
			for (int i = 0; i < PARTICLES_PER_POINT; ++i) {
				...
				Particle particle = new Particle(debris, paint, PARTICLE_MAX_RADIUS);
				particle.dx = randsgn() * (float) Math.random() * PARTICLES_MAX_MULT;
				particle.dy = randsgn() * (float) Math.random() * PARTICLES_MAX_MULT;
				mParticles.add(particle);
			}
		}
		...
		private synchronized void onTouch(MotionEvent e) {
